function addToCart(itemId, def, color, material, layerHeight) {
    ...
    if (def) {
        ...
        url = `http://localhost/?&itemId=${itemId.toString()}&color=${color}&material=${material}&layerHeight=${layerHeight}&c=cart&a=addToCart`;
    }
...
function addToOrder(id) {
    let url = `http://localhost/?&id=${id.toString()}&c=cart&a=addToOrder`;
    let request = new XMLHttpRequest();
    request.onload = function () {
